...
_slackbot.prototype.post = function(msg, channel, from, attachment, icon, callback) {
		...
		// make sure the caller configured the @'s correctly
		if( channel.charAt(0) !== '@' && channel.charAt(0) !== '#' ) {
				channel = "@" + channel;
		} else if( channel.charAt(0) === '@' && channel.charAt(1) === '@' ) {
		...
		request(request_params, function(error, response, body) {
		  	if( error ) {
		  		...
		  		logme.inspect(error);
					...
					logme.debug('Slackbot post returned error ' + response.statusCode);
					logme.inspect(body);
				}
