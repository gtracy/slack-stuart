...
module.exports.run = function(request, cmd_args, stuart, plugin) {
		...
		client.availablePhoneNumbers('US').local.get({
		    ...
		    if (searchResults.availablePhoneNumbers.length < 1) {
				...
				var conf_name = request.user_name + 'slack' + num.slice(1);
				// setup the unique call handler for this line
				var voice_url = encodeURI(plugin.config.host + '/slack/conference?'
				    + 'name=' + conf_name
				    + '&password=' + password
        ...
        client.incomingPhoneNumbers.create({
            ...
            if (buyError) {
                ...
                stuart.slack_post("Oh noes! We couldn't buy your number. :( Try again in a bit.", '@'+request.user_name);
            } else {
			  ...
			  logme.debug('CRON task running to cleanup old conference lines...');
				client.incomingPhoneNumbers.list(function(err, data) {
				    ...
				    data.incoming_phone_numbers.forEach(function(number) {
			        	var age = (Date.now() - new Date(number.date_created)) / (1000*60*60*24);
								if( number.friendly_name.indexOf(CONFERENCE_LINE_NAME_PREFIX) >= 0 ) {
							    	...
							    	// ************************************************************** //
						      	logme.debug('... ' + number.friendly_name + ' : ' + number.date_created + ', age : ' + age);
										logme.debug('... ' + new Date(number.date_created));
							    	if( age > plugin.config.max_age ) {

									    	logme.info('Deleting conference line : ' + number.friendly_name+" : "+number.phone_number+" : "+age+" : "+number.date_created);
									    	client.incomingPhoneNumbers(number.sid).delete(function(err) {
										    		if( err ) {
											    			logme.error('Twilio delete fail');
											    			logme.inspect(err);
										    		} else {
															  logme.info('... deletion success');
														}
									    	});
								    } else {
									    	logme.debug('... skip');
