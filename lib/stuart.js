...
function _Stuart() {
	...
	this.slack_post = function(msg, channel, from) {
		this.slackbot.post(msg,channel,from);
	};
...
_Stuart.prototype.slash_command = function(request) {
	logme.info(request.text);
	var cmd = request.text.split(' ')[0];

    // Service a HELP requests
    //
	if( cmd.toLowerCase() === "help" ) {

		var output = "Those crazy devs are always adding new tricks, but here's what I've got right now...\n\n";
		var stuart_commands = this.commands;
		_.keys(this.commands).forEach(function(key) {
			output += key + " :: " + stuart_commands[key].name + "\n\n";
		});
		output += "\n\nIf you'd like to dig deeper, type '/stuart <cmd> help' for individual commands";
		...
		if( _.isEmpty(plugin) ) {
			...
			try {
				...
				if( meta.indexOf('help') >= 0 ) {
					...
					this.emit(cmd, request, this, plugin);
				}
    ...
    if (err) {
        ...
        _.keys(stuart.commands).forEach(function(cmd) {
            ...
            if( stuart.commands[cmd].active === false ) {
                logme.info(cmd + " is configured to be inactive. Update the plugin spec to change this");
        ...
        logme.inspect(stuart.cron);
        _.keys(stuart.cron).forEach(function(task) {

            if( stuart.cron[task].active === false ) {
                logme.info(task + " is configured to be inactive. Update the plugin spec to change this");
